...
namespace ShiftCo.ifmo_ca_lab_3.Evaluation.Util
{
    public class ElementComparer : Comparer<IElement>
    {
        public override int Compare(IElement left, IElement right)
        {
            ...
            if (left is Expression le && right is Expression re && le.Elements.Count != re.Elements.Count)
            {
                return le.Elements.Count - re.Elements.Count;
            }
            // Compare each operand
            var zipedOperands = ((Expression)left).Elements.Zip(((Expression)right).Elements,
